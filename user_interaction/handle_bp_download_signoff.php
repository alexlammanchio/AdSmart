...
if (isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT bp_filename FROM quotation_content WHERE quotation_id = ?");
    ...
    if ($row1) {
        ...
        $filepath = '../pdf/business_partner/'.$row1['bp_filename']; // Fixed to use 'bp_filename'
        ...
        if (file_exists($filepath)) {
            ...
            header('location:'.USER_INTERACTION."Adsmart_partners_after_created_quotation.php?id=".$file_id);
            exit;
        }
    } else {
