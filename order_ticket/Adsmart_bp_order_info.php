...
<div style="background:#1b0075">
 ...
 <!-- item information  --> 
			...
			$sql2 = "SELECT *,a.quantity as payment_quantity, a.company_user_id from payment a join adsmart_business_product b on a.item_name = b.product_name where a.company_user_id = '$account_name'  AND transaction_id ='$tran_id' And a.req_number ='' ";
		...
		?>
			        ...
			        $tran_id = $_GET['trans_id'];
			        $sql3 = "SELECT * from payment a join quotation_content b on a.req_number = b.req_number where a.company_user_id = '$account_name'  AND a.transaction_id ='$tran_id' AND a.req_number !=''";
...
<h1>Cutomer Information</h1>
			...
			<?php 
			$sql3 = "SELECT * from payment a join adsmart_customer b on a.cus_user_id = b.user_id where a.company_user_id = '$account_name'";
			$res3 = mysqli_query($conn, $sql3);			
			        ...
			        $row2=mysqli_fetch_assoc($res3);
			            ...
			            echo "<label>Transaction Date</label>:<b>".$row2['created_day']."</b><br>";
			            ?>
		...
		<h1>Company's Information</h1>
			...
			<?php 
			$sql4 = "SELECT * from payment a join adsmart_business_partner b on a.company_user_id = b.user_id where transaction_id = '$tran_id' AND a.company_user_id ='$account_name'";
			...
			if($res4==TRUE){
			    ...
			    if($count1>0){
			        while($row4=mysqli_fetch_assoc($res4))
			        {
			            ...
			            if($status1 != ''){
			            echo "<label>Current Status</label>:<b>". $status1."</b><br>";	
			            }else{
			                echo "<label>Current Status</label>:<b>Waiting for Vendor Update !</b><br>";	
			            }
			            if($status1 != 'completed' OR $status1 == ''){	 
			                echo "<form action='handle_update_bp_payment_status.php' method='post'>";
			                echo "<input type='hidden' name='tran_id' value=".$tran_id1.">";
			                echo "<input type='hidden' value='" . htmlspecialchars($item_name1, ENT_QUOTES, 'UTF-8') . "' name='item_name' >";			                
			                echo  "<input type='hidden' value=".$company_user_id1." name='company_user_id' >";
			               
			                echo "<label>Status:</label>";
			                echo "<select required name='status'  >";
			                echo "<option value=''>Select One</option>";
			                echo "<option value='prepare'>Preparing</option>";
			                echo "<option value='delivery'>Devlierying</option>";
			                echo "<option value='pending_for_cus'>Waiting Customer to Confirm</option>";
			                echo "</select>";
			                echo "<br>";
			                echo "<input  style='background:#096f04; color:white; margin-top:10px; width:300px; height:35px; text-align:center; font-size:25px;border-radius: 10px;' type='submit' value='Update' name='submit'>";
			                echo "</form>";
			                echo "<br>";
			            }
