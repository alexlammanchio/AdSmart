...
<!-- -Cart Items Details -->
<div class="small-container cart-page " style ="padding-bottom:600px;">
		...
		<h1>Completed Order Items</h1>             
           ...
           $start = ($pageNum - 1) * $limit; // 计算当前页面的起始记录
            $sql= "select * from payment where company_user_id = '$account_name' AND order_status ='completed' LIMIT $start, $limit";
			...
			if ($rows['transaction_id'] !== $previousTransactionId) {
			    ...
			    echo "<td>";
			    echo "<a href='" . ORDER_TICKET . "Adsmart_bp_order_info.php?trans_id=" . $rows['transaction_id'] . "'>";
			...
			$previousTransactionId = $rows['transaction_id']; // Update the previous transaction ID
                ...
                $countQuery = "SELECT COUNT(id) AS cnt from payment where company_user_id = '$account_name' AND order_status ='completed' "; // 替換 your_table 為您的數據表名
