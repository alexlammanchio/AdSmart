...
<div class="small-container cart-page " style ="padding-bottom:700px;">
		...
		<h1>Check quotaiton Item Status</h1>             
           ...
           $sql1 = "select a.req_number from quotation_content a join payment b on a.req_number = b.req_number where b.company_user_id = '$account_name'LIMIT $start, $limit";
           ...
           if (!empty($excludedReqNumbers)) {
            ...
            if($res==TRUE){
				   ...
				   <th> Action</th>
                    ...
                    while($rows=mysqli_fetch_assoc($res))
                    {
                        ...
                        if ($rows['req_number'] !== $previousTransactionId) {
                            ...
                            echo "<td>";
                            echo "<a href='" . ORDER_TICKET . "Adsmart_check_quotation_info.php?req_number=" . $rows['req_number'] . "'>";
                ...
                // 計算總頁數
                $countQuery = "SELECT COUNT(a.id) AS cnt from quotation_content a join payment b on a.req_number = b.req_number where b.company_user_id = '$account_name' "; // 替換 your_table 為您的數據表名
    ...
    <?php if ($pageNum > 1): ?>
        <SPAN style= 'width:65px;'><a href="?page=9&pageNum=<?php echo $pageNum - 1; ?>">Previous</a></SPAN>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
       <SPAN> <a href="?page=9&pageNum=<?php echo $i; ?>"><?php echo $i; ?></a></SPAN>
    <?php endfor; ?>

    <?php if ($pageNum < $totalPages): ?>
       <SPAN> <a href="?page=9&pageNum=<?php echo $pageNum + 1; ?>">Next</a></SPAN>
    <?php endif; ?>
</div>
				</div>
		</div>
