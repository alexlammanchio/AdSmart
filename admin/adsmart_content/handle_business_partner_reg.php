...
if(isset($_POST['submit'])){
   ...
   if(isset($_POST['print_ads'])){
    ...
    //1. Get the data from form
    $contact_number =$_POST['company_reg_address'];
    $_SESSION['company_reg_address'] = $_POST['company_reg_address'];
    // 定義不允許字元
    $specialChars = "!@#$%^&*()/\\\'\"";
    ...
    if(strpbrk($country, $specialChars) !== false) {
        // special char found
        $_SESSION['special_character3'] = "<div style='color:red; font-size:14px;'> Email just accept Hyphen - and Underscore _ special characters. </div>";
        header('location:'.SITEURL.'Adsmart_partners_registration.php');
        die();
    }
