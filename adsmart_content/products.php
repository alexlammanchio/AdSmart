			         ...
			         while($row =mysqli_Fetch_assoc($res)){
						...
						<div class="col-4">
			             <?php 
			               ...
			               <a href="<?php echo SITEURL;?>explore-business.php?id=<?php  echo $shop_code;?>"   target="blank"> <button type="button"class="btn" style="background:#9198e5">Detail</button> </a>
...
FROM (
    SELECT *,
        CASE WHEN print_ads = 1 THEN 'print_ads' ELSE NULL END as print_ads1,
        CASE WHEN outdoor_ads = 1 THEN 'outdoor_ads' ELSE NULL END as outdoor_ads1,
        CASE WHEN broadcast_ads = 1 THEN 'broadcast_ads' ELSE NULL END as broadcast_ads1,
        CASE WHEN telemarketing_ads = 1 THEN 'telemarketing_ads' ELSE NULL END as telemarketing_ads1,
        CASE WHEN events_ads = 1 THEN 'events_ads' ELSE NULL END as events_ads1,
        CASE WHEN placement_ads = 1 THEN 'placement_ads' ELSE NULL END as placement_ads1,
        CASE WHEN display_ads = 1 THEN 'display_ads' ELSE NULL END as display_ads1,
        CASE WHEN social_ads = 1 THEN 'social_ads' ELSE NULL END as social_ads1,
        CASE WHEN search_ads = 1 THEN 'search_ads' ELSE NULL END as search_ads1,
        CASE WHEN video_ads = 1 THEN 'video_ads' ELSE NULL END as video_ads1,
        CASE WHEN native_ads = 1 THEN 'native_ads' ELSE NULL END as native_ads1,
        CASE WHEN influencer_ads = 1 THEN 'influencer_ads' ELSE NULL END as influencer_ads1
    FROM adsmart_business_partner
) t 
WHERE '$category_name' IN (
    print_ads1, outdoor_ads1, broadcast_ads1, telemarketing_ads1, events_ads1,
    placement_ads1, display_ads1, social_ads1, search_ads1, video_ads1,
    native_ads1, influencer_ads1
) "; // 替換 your_table 為您的數據表名
    ...
    <?php if ($pageNum > 1): ?>
        <SPAN style= 'width:65px;'><a href="?ads_id=<?php echo $type_id;?>&pageNum=<?php echo $pageNum - 1; ?>">Previous</a></SPAN>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
       <SPAN> <a href="?ads_id=<?php echo $type_id;?>&pageNum=<?php echo $i; ?>"><?php echo $i; ?></a></SPAN>
    <?php endfor; ?>

    <?php if ($pageNum < $totalPages): ?>
       <SPAN> <a href="?ads_id=<?php echo $type_id;?>&pageNum=<?php echo $pageNum + 1; ?>">Next</a></SPAN>
    <?php endif; ?>
</div>
				</div>	
